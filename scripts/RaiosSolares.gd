...
#
var material := preload("res://texturas/RaiosDeSolShader.tres").duplicate() as ShaderMaterial
...
#	var light := get_node(light) as Light
	...
	#padrão sample = 20.0, dither = 1.0, pcf5 = true
	material.set_shader_param("num_samples", 20)
	material.set_shader_param("use_pcf5", true)
	material.set_shader_param("dither", 1)
